...
public class ClienteRegistroController {
    ...
    @FXML
    private TextField txtNombres, txtApellidos, txtTelefono;

    @FXML
    private ComboBox<String> cmbTipoCliente, cmbCiudad;
    ...
    @FXML
    private CheckBox chkInternet, chkTelefonia, chkSoporte;
    ...
    @FXML
    private void avanzarApellidos(ActionEvent e) {
        txtApellidos.requestFocus();
    }

    @FXML
    private void avanzarTelefono(ActionEvent e) {
        txtTelefono.requestFocus();
    }

    @FXML
    private void avanzarTipoCliente(ActionEvent e) {
        cmbTipoCliente.requestFocus();
    }

    @FXML
    private void avanzarCiudad(ActionEvent e) {
        cmbCiudad.requestFocus();
    }

    @FXML
    private void avanzarFecha(ActionEvent e) {
        dpFechaNacimiento.requestFocus();
    }

    @FXML
    private void avanzarSolicitud(ActionEvent e) {
        rbInformacion.requestFocus();
    }
    ...
    public void initialize() {
        ...
        dpFechaNacimiento.setDayCellFactory(p -> new DateCell() {
                    @Override
        ...
        txtNombres.textProperty().addListener(listener);
        txtApellidos.textProperty().addListener(listener);
        txtTelefono.textProperty().addListener(listener);
        cmbTipoCliente.valueProperty().addListener(listener);
        cmbCiudad.valueProperty().addListener(listener);
        dpFechaNacimiento.valueProperty().addListener(listener);
        grupoSolicitud.selectedToggleProperty().addListener(listener);
        chkInternet.selectedProperty().addListener(listener);
        chkTelefonia.selectedProperty().addListener(listener);
        chkSoporte.selectedProperty().addListener(listener);
    ...
    @FXML
    private void seleccionarFotografia(ActionEvent e) {
        FileChooser chooser = new FileChooser();
        chooser.setTitle("Seleccionar fotografía del cliente");
        ...
        if (archivo != null) {
            ...
            lblEstado.setText("Fotografía seleccionada: " + archivo.getName());
        }
    }
    ...
    private void guardarCliente(ActionEvent e) {
        ...
        Sesion.clientes.add(cliente);
        limpiar();

        lblEstado.setText("Cliente guardado correctamente. Puedes registrar el siguiente.");
    ...
    private void actualizarEstadoBoton() {
        boolean formularioIncompleto =
                ...
                || txtTelefono.getText().trim().length() != 8
        ...
        btnRegistrar.setDisable(formularioIncompleto);
    }

    private void aplicarFormato(TextField campo, String patron) {
        UnaryOperator<TextFormatter.Change> filtro = c ->
                c.getControlNewText().matches(patron) ? c : null;
    ...
    private boolean validar() {
        if (txtNombres.getText().trim().isEmpty())
            return error("Ingresa los nombres del cliente.", txtNombres);

        if (txtApellidos.getText().trim().isEmpty())
            return error("Ingresa los apellidos del cliente.", txtApellidos);

        if (txtTelefono.getText().length() != 8)
            return error("El teléfono debe contener exactamente 8 dígitos.", txtTelefono);

        if (cmbTipoCliente.getValue() == null)
            return error("Selecciona el tipo de cliente.", cmbTipoCliente);

        if (cmbCiudad.getValue() == null)
            return error("Selecciona una ciudad.", cmbCiudad);

        if (dpFechaNacimiento.getValue() == null)
            return error("Selecciona la fecha de nacimiento.", dpFechaNacimiento);

        return grupoSolicitud.getSelectedToggle() != null || error(
                "Selecciona el tipo de solicitud.", rbInformacion);
    }

    private boolean error(String texto, Node campo) {
    ...
    private List<String> servicios() {
        ...
        if (chkInternet.isSelected()) r.add(chkInternet.getText());
        if (chkTelefonia.isSelected()) r.add(chkTelefonia.getText());
        if (chkSoporte.isSelected()) r.add(chkSoporte.getText());
        return r;
    }
    ...
    @FXML
    private void abrirMenu(ActionEvent e) {
        SceneManager.cambiarEscena(stage(),
                RUTA_MENU,
                "Menú Principal");
    }
    ...
    @FXML
    private void acercaDe(ActionEvent e) {
